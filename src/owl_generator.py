...
class OWLGenerator:
    ...
    def _clean_fragment(value: str) -> str:
        """Sanitize a string for safe use in URI fragments."""
    ...
    def generate_ontology(self):
        ...
        used_property_fragments: set[str] = set()
        ...
        for entity in attributes_data:
            ...
            for attr, attr_type in entity["attr"].items():
                attr_fragment_root = f"{self._clean_fragment(entity['name'])}_{self._clean_fragment(attr)}"
                attr_fragment = attr_fragment_root
                counter = 1
                while attr_fragment in used_property_fragments:
                    attr_fragment = f"{attr_fragment_root}_{counter}"
                    counter += 1
                used_property_fragments.add(attr_fragment)
                attr_uri = URIRef(f"{self.owl_ns}{attr_fragment}")
                self.graph.add(
                    (
                        attr_uri,
                        RDF.type,
        ...
        for relation in valid_relations:
            ...
            relation_fragment_root = self._clean_fragment(relation["relation"])
            relation_fragment = relation_fragment_root
            counter = 1
            while relation_fragment in used_property_fragments:
                relation_fragment = f"{relation_fragment_root}_obj{counter}"
                counter += 1
            used_property_fragments.add(relation_fragment)
            relation_name = relation_fragment
            ...
            self.graph.add((relation_uri, RDFS.label, Literal(relation_label)))
