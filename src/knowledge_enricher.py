...
class KnowledgeEnricher:
    ...
    def __init__(
        ...
        if input_path is None:
            input_path = str(
                Path(__file__).resolve().parents[1] / "output" / "ontology.owl"
            )

        if output_path is None:
            output_path = str(
                Path(__file__).resolve().parents[1] / "output" / "enriched_ontology.owl"
            )
    ...
    def _train_and_generate_candidates(
        ...
        try:
            with Progress() as progress:
                ...
                progress.update(task, completed=45)

                # Reduce training complexity, decrease training time
                num_epochs = min(20, 200)  # Max 50 epochs, not 200
                batch_size = (
                ...
                console.print(
                    "[cyan]Running PyKEEN training... first run may take a little longer while caches are prepared.[/]"
                )
                progress.update(
                    task,
                    completed=50,
                    description="[cyan]Training knowledge graph embeddings (PyKEEN)...[/]",
                )

                # Train model - use TransE (simplest and most efficient model)
                pipeline_stats.start_timer("transe_training")
                try:
                    with ThreadPoolExecutor(max_workers=1) as executor:
                        ...
                        interim_progress = 50.0
                        while True:
                            try:
                                result = future.result(timeout=1.0)
                                break
                            except FuturesTimeoutError:
                                interim_progress = min(interim_progress + 2.0, 78.0)
                                progress.update(
                                    task,
                                    completed=interim_progress,
                                )
                            except Exception:
                                future.cancel()
                                raise
                finally:
                    pipeline_stats.stop_timer("transe_training")
                ...
                # Prepare candidate indices for batch scoring
                    ...
                    for t in nodes:
                        ...
                        for rel in relations_list:
                            if (h, rel, t) in existing_triples:
                                continue
                            candidate_indices.append([h_id, relation_to_id[rel], t_id])
                            candidate_triples.append((h, rel, t))
                ...
                if total_candidates:
                    chunk_size = 256
                    scored = 0
                    scoring_budget = 13.0  # leave room for finalization

                    with torch.no_grad():
                        for start in range(0, total_candidates, chunk_size):
                            end = start + chunk_size
                            batch_slice = candidate_indices[start:end]
                            batch = torch.tensor(
                                batch_slice, dtype=torch.long, device=self.device
                            )
                            try:
                                ...
                                scores = np.zeros(len(batch_slice), dtype=float)

                            for idx, score in enumerate(scores):
                                h, rel, t = candidate_triples[start + idx]
                                candidates.append((float(score), h, rel, t))

                            scored += len(batch_slice)
                            fraction = scored / total_candidates
                            progress.update(
                                task,
                                completed=min(
                                    85 + scoring_budget * fraction,
                                    98,
                                ),
                            )
                else:
