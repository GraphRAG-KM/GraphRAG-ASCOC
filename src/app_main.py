...
from stats import pipeline_stats, capture_openai_usage
...
def main_entry(input_pdf: Optional[str] = None):
    ...
    if input_pdf is not None:
        ...
        pipeline_stats.record_file_size("input_pdf", input_pdf_path)
    else:
    ...
    with Progress(
        ...
        if md_files:
            total_md_size = sum(md_file.stat().st_size for md_file in md_files)
            pipeline_stats.record_file_size(
                "markdown_total", size_bytes=total_md_size
            )
    ...
    pipeline_stats.start_timer("graphrag_index")
    try:
        with capture_openai_usage("graphrag_index"):
            asyncio.run(build_graphrag_index(graphrag_config))
    finally:
        pipeline_stats.stop_timer("graphrag_index")
    ...
    summary_lines = pipeline_stats.formatted_summary()
    if summary_lines:
        console.print("[cyan]===== Pipeline Metrics =====[/]")
