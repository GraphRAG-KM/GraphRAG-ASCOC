...
class InferenceProcessor:
    def __init__(
        ...
        if parquet_input_path is None:
            parquet_input_path = Path(__file__).resolve().parents[1] / "output"
    ...
    async def _infer_entity_attributes(self, title, desc, progress, task):
        ...
        pipeline_stats.record_token_usage(
            self.config.chat_model_name, getattr(response, "usage", None), "entity_attr"
        )
    ...
    def _merge_synonyms(self, embeddings, threshold=0.9, target_entity_count=None):
        ...
        pipeline_stats.start_timer("merge_synonyms")
        try:
            ...
            pipeline_stats.stop_timer("merge_synonyms")
